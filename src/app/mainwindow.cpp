...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->pushButton_build, SIGNAL(clicked()), this,
          SLOT(on_pushButton_build_clicked()));
}
...
void MainWindow::on_pushButton_build_clicked() {
  ...
  if (ui->lineEdit_x_min->text().toDouble() >=
      ...
      (ui->lineEdit_x_min->text().toDouble() < -1e6) ||
      (ui->lineEdit_x_max->text().toDouble() > 1e6)) {
    QMessageBox::warning(this, "Внимание",
                         "Область определения указана неправильно.");
    return;
  }
  if (ui->lineEdit_y_min->text().toDouble() >=
          ui->lineEdit_y_max->text().toDouble() ||
      (ui->lineEdit_y_min->text().toDouble() < -1e6) ||
      (ui->lineEdit_y_max->text().toDouble() > 1e6)) {
    QMessageBox::warning(this, "Внимание",
                         "Область значения указана неправильно.");
    return;
  }
  delete my_graph;
  my_graph = new graph();
  ...
  emit signal(ui->lineEdit_str->text(), ui->lineEdit_x_min->text().toDouble(),
              ui->lineEdit_x_max->text().toDouble(),
              ui->lineEdit_y_min->text().toDouble(),
              ui->lineEdit_y_max->text().toDouble());
  my_graph->show();
}
